import { db, publishOccupation, approveQueueItem } from '../lib/db';
import { resolveFundingEligibility, FundingEligibilityRow } from '../lib/funding';
...
function runPhase2AcceptanceTests() {
  ...
  // --- ACCEPTANCE TEST 1: TIER-A PUBLISH TRIGGER GATE ---
  console.log('>>> ACCEPTANCE TEST 1: TIER-A PUBLISH TRIGGER GATE <<<');
  try {
    // Seed Tier-A occupation with an unverified step
    db.exec(`
      INSERT OR REPLACE INTO occupations (id, title, summary, tier, published, confidence)
      VALUES ('occ-tier-a-test', 'Tier A Test Occupation', 'Summary', 'A', 0, 'confirmed');

      INSERT OR REPLACE INTO routes (id, occupation_id, type, label, confidence, verified_by, verified_at)
      VALUES ('route-tier-a-test', 'occ-tier-a-test', 'college', 'College Track', 'confirmed', 'pete@vocari.co.uk', CURRENT_TIMESTAMP);
      ...
      VALUES ('step-unverified-test', 'route-tier-a-test', 1, 'Unverified Step 1', 'provisional', NULL, NULL);
    `);

    console.log('Attempting to publish Tier-A occupation with unverified step...');
    publishOccupation('occ-tier-a-test');
    console.error('❌ FAIL: Publish succeeded when it should have been blocked by Tier-A gate!');
  } catch (error: any) {
    console.log('✅ PASS: Database trigger / constraint fired successfully:');
    console.log(`   ERROR VERBATIM: "${error.message}"\n`);
  }

  // --- ACCEPTANCE TEST 2: APPROVE QUEUE ITEM & REVIEW_DUE SETTING ---
  ...
  db.exec(`
    ...
    VALUES ('req-queue-test', 'qualification', 'L3 Diploma In Electrical', 'provisional', NULL, NULL, NULL);
  `);
  ...
  // --- ACCEPTANCE TEST 3: DUAL FUNDING SCHEME RESOLVER (ALL vs LLE) ---
  console.log('>>> ACCEPTANCE TEST 3: DUAL FUNDING SCHEME RESOLVER (ALL vs LLE) <<<');
  const sampleFundingRows: FundingEligibilityRow[] = [
    {
      id: 'fe-all-row',
      ...
      scheme_valid_to: '2027-12-31', // Extended to 2027
    },
    {
      id: 'fe-lle-row',
      qualification_id: 'qual-lle-all-test',
      ...
      covers: 'loan_only',
      learner_contribution_gbp: 0,
      scheme_valid_from: '2027-01-01', // Valid for courses starting on or after Jan 1, 2027
  ...
  if (
    resNov2026.eligibleSchemes.some((s) => s.scheme === 'advanced_learner_loan') &&
    resFeb2027.eligibleSchemes.some((s) => s.scheme === 'lifelong_learning_entitlement')
  ) {
    console.log('\n✅ PASS: Concurrent funding schemes resolved correctly based on course start date!\n');
  } else {
    console.error('❌ FAIL: Funding resolution error!');
  }
}
